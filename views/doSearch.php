...
<?php 
	$typevalue = $_POST['seltype'];
	$locvalue = $_POST['sellocation'];
...
<?php
    ...
    if(strcmp("Egypt",$locvalue) == 0){
		$query = mysql_query("SELECT * from workers 
    								...
    								WHERE workers.typeid = $typevalue AND workers.name LIKE '%$workername%'"); 
    }else{
		$query = mysql_query("SELECT * from workers 
    								INNER JOIN types ON workers.typeid=types.id 
    								INNER JOIN workerslocations on workerslocations.workerid = workers.id
    								INNER JOIN phones on phones.workerid = workers.id
    								INNER JOIN places on places.id = workerslocations.id
    								WHERE workers.typeid = $typevalue AND places.place = '$locvalue' AND workers.name LIKE '%$workername%'"); 
    }
